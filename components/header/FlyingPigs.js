...
export const FlyingPigs = () => {
  ...
  React.useEffect(() => {
    ...
    function Tunnel (pw, ph) {
      ...
      this.maxA = 0.6 // greatest slope of walls
      this.maxAVar = 0.05 // maximum angle variance between steps
      this.wColor = '#fff' // wall color
      this.oColor = '#fff' // obstacle color
      this.bColor = '#8363ae' // background color
      this.startingGap = 0.5 // starting ratio of gap to height
      this.maxObCoeff = 3
      this.minObSize = 23
      this.obWidth = 20
      this.obDist = 120
      ...
      this.move = function () {
        ...
        for (var i = 0; i < this.rects.length; i++) {
          ...
          if (currRect.x < -this.resolution) {
            ...
            if (currRect.y == 0) {
              ...
              if (currRect.x >= this.w - this.resolution)
                this.lastTopRect = currRect
            } else {
              // it's the lower
        ...
        if (this.lastTopRect && this.sinceLastOb > this.obDistNext) {
          newH =
            this.minObSize * (1 + parseInt(Math.random() * this.maxObCoeff))
          ...
          this.sinceLastOb = 0
          this.obDistNext =
            this.obDist + (Math.random() * 2 * this.obDistVar - this.obDistVar)
        }
      }
      ...
      this.init = function (w, h) {
        ...
        for (var col = 0; col < colNum; col++) {
          this.rects[2 * col] = new Rect(
            col * this.resolution,
            0,
            this.resolution,
            this.currOffset
          ) // top section
          this.rects[2 * col + 1] = new Rect(
            col * this.resolution,
            this.currOffset + this.currGap,
            this.resolution,
            this.h - (this.currOffset + this.currGap)
          ) // bottom section
          this.updateOffset()
        }
      }
    ...
    function Pig (px, py) {
      ...
      Rect.prototype.constructor.call(this, px, py, 100, 65)

      // cap velocities
      this.capV = function () {
        if (Math.abs(this.velocity.y) > Math.abs(this.maxVelocity.y)) {
          this.velocity.y =
            (this.velocity.y / Math.abs(this.velocity.y)) * this.maxVelocity.y
        }
      }
  ...
  return (
    <div>
      <canvas id='screen' width={width} height='750'></canvas>
    </div>
  )
}
